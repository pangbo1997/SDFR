...
from icp_module.cosypose_icp.icp_refiner_modified import getXYZ
...
def main(configs):
    ...
    if configs['use_icp']:
        ...
        from icp_module.ICP_Cosypose import ICPRefiner
    ...
    zebra_dataset_dir='zebra_dataset/{}/obj_{:06d}'.format(dataset_name,obj_id+1)
    os.makedirs(f'{zebra_dataset_dir}/color',exist_ok=True)
    os.makedirs(f'{zebra_dataset_dir}/mask',exist_ok=True)
    os.makedirs(f'{zebra_dataset_dir}/trg_pcds',exist_ok=True)
    os.makedirs(f'{zebra_dataset_dir}/src_pcds',exist_ok=True)
    os.makedirs(f'{zebra_dataset_dir}/gt_poses',exist_ok=True)
    os.makedirs(f'{zebra_dataset_dir}/init_poses',exist_ok=True)
    os.makedirs(f'{zebra_dataset_dir}/cam_K',exist_ok=True)
    ...
    for batch_idx, (data, entire_masks, masks, Rs, ts, Bboxes, class_code_images, cam_Ks) in enumerate(tqdm(test_loader)):
        ...
        for counter, (r_GT, t_GT, Bbox, cam_K) in enumerate(zip(Rs, ts, Bboxes, cam_Ks)):
            ...
            if success:     
                ...
                cv2.imwrite(f'{zebra_dataset_dir}/mask/{batch_idx}.png',full_mask*255)
                ...
                full_mask = compute_original_mask(Bbox, test_img.shape[0], test_img.shape[1], pred_masks[counter])
                depth_image=depth_image*full_mask
                depth_image=background_filter(torch.tensor(depth_image),obj_diameter).squeeze().numpy()

                depth_measured=depth_image/1000
                cam_K=cam_K.cpu().detach().numpy()
                points_tgt = np.zeros((depth_measured.shape[0],depth_measured.shape[1],3),np.float32)
                ...
                depth_valid = np.logical_and(depth_valid,full_mask)
                points_tgt = points_tgt[depth_valid]

                trg_pcd=o3d.geometry.PointCloud()
                trg_pcd.points=o3d.utility.Vector3dVector(points_tgt)
                src_pcd=o3d.geometry.PointCloud()
                src_pcd.points=o3d.utility.Vector3dVector((vertices@r_GT.T+t_GT[:,0])/1000)
                ...
                trg_pcd.points=o3d.utility.Vector3dVector(trg_pts)
                src_pcd.points=o3d.utility.Vector3dVector(src_pts)


                o3d.io.write_point_cloud(f'{zebra_dataset_dir}/trg_pcds/{batch_idx}.ply',trg_pcd)
                o3d.io.write_point_cloud(f'{zebra_dataset_dir}/src_pcds/{batch_idx}.ply',src_pcd)
                cv2.imwrite(f'{zebra_dataset_dir}/color/{batch_idx}.png',cv2.imread(test_rgb_files[obj_id][batch_idx]))
                gt_pose=np.zeros((4,4))
                gt_pose[:3,:3]=r_GT
                gt_pose[:3,3]=t_GT[:,0]
                np.savetxt(f'{zebra_dataset_dir}/gt_poses/{batch_idx}.txt',gt_pose)
                pred_pose=np.zeros((4,4))
                pred_pose[:3,:3]=R_predict
                pred_pose[:3,3]=t_predict[:,0]
                np.savetxt(f'{zebra_dataset_dir}/init_poses/{batch_idx}.txt',pred_pose)         

                np.savetxt(f'{zebra_dataset_dir}/cam_K/{batch_idx}.txt',cam_K)        
